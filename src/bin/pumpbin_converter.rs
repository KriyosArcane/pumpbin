...
fn main() -> Result<()> {
    ...
    println!(
        "   - Max Len: {}",
        1024 * 1024 * config.shellcode_size_mb + 13
    );
    println!("   - Type: Local");
...
fn analyze_project(project_path: &Path) -> Result<ProjectAnalysis> {
    ...
    let main_rs_path = main_rs_candidates
        .into_iter()
        .find(|p| p.exists())
...
fn detect_shellcode_patterns(content: &str) -> Vec<ShellcodePattern> {
    ...
    if content.contains("fs::read") || content.contains("File::open") {
        patterns.push(ShellcodePattern {
            ...
            suggestions: vec!["Replace with PumpBin placeholder system".to_string()],
        });
    }
    ...
    if content.contains("VirtualAlloc") || content.contains("mmap") {
        patterns.push(ShellcodePattern {
            ...
            suggestions: vec!["Good - keep this execution logic unchanged".to_string()],
        });
    }
...
fn detect_target_os(main_content: &str, cargo_content: &str) -> Vec<String> {
    ...
    if main_content.contains("windows_sys")
        || main_content.contains("winapi")
        || main_content.contains("VirtualAlloc")
        || cargo_content.contains("windows")
    {
        targets.push("Windows".to_string());
    }

    if main_content.contains("libc")
        || main_content.contains("mmap")
        || main_content.contains("#[cfg(target_os = \"linux\")]")
    {
        targets.push("Linux".to_string());
    }

    if main_content.contains("#[cfg(target_os = \"macos\")]") || main_content.contains("darwin") {
...
fn display_analysis(analysis: &ProjectAnalysis) {
    ...
    println!(
        "  🔧 Has build.rs: {}",
        if analysis.has_build_rs { "Yes" } else { "No" }
    );
...
fn main() {{
    ...
    shellcode.extend(iter::repeat_n(b'0', {}));
...
fn replace_shellcode_patterns(mut content: String) -> String {
    ...
    content = include_bytes_regex
        .replace_all(&content, r#"include_bytes!("../shellcode")"#)
        .to_string();
    ...
    if file_read_regex.is_match(&content) {
        ...
        content = file_read_regex
            .replace_all(
                &content,
                "/* TODO: Replace with PumpBin shellcode loading */\n    get_shellcode().to_vec()",
            )
            .to_string();
    }
